...
//    private final String HOME_PAGE = "HomePageServlet";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            AccountDTO dto = new AccountDAO().checkLogin(username, password);
            if (dto != null) {
                if (dto.getRoleID().equals("AD")) {
                    url = "AdminPageServlet?pageIndex=1";
                    session.setAttribute("user", dto);
                }
                if (dto.getRoleID().equals("US")) {
                    url = "HomePageServlet?pageIndex=1";
                    session.setAttribute("customer", dto);
                }

            } else {
                request.setAttribute("error", "Username or password incorrected!");
            }
        } catch (SQLException | NamingException e) {
            log.error("LoadPage error : " + e.toString());
        } finally {
